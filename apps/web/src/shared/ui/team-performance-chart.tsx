...
export function TeamProgressWithLabel({ teamId, teamTitle, averageRating, employeesCount, color }: TeamPerformanceData) {
    return (
        <Field className="w-full">
            <FieldLabel htmlFor={teamId.toString()}>
                <span className="font-semibold text-base">{teamTitle}</span>
                <span className="text-sm text-muted-foreground border border-black-500 rounded-lg px-1.5 py-0.5 ml-2">{employeesCount} {employeesCount === 1 ? 'employee' : 'employees'}</span>
                <span className="ml-auto flex flex-row gap-1 items-center justify-center">
                    <span className="font-semibold">{formatNumber(averageRating, 2)}</span>
                    <Percent className="shrink-0 h-4 w-4 text-muted-foreground" />
                </span>
            </FieldLabel>
            <Progress value={averageRating} id={teamId.toString()} indicatorClassName={color ? color : 'bg-emerald-400'} />
        </Field>
    )
}
    ...
    return (
        ...
        <Card className="flex-1 min-w-[95px] w-full overflow-hidden">
            <CardHeader className="items-center">
                    ...
                    <CardDescription>{description || 'Comparison of average ratings across different organizational units. This view allows leadership to identify high-performing teams and departments that may require additional support or resource optimization.'}</CardDescription>
            </CardHeader>
