    ...
    const reportsByStage = useMemo(() => {
        ...
        const filteredReports = allReportsData
            ...
            .filter((r) => {
                ...
                if (isTeamReports) {
                    return review?.managerId === currentUser.user.id ? r : null;
                }
                return r;
            })
        ...
        buckets['NONE'] = filteredReports.filter(
                        ...
                        const reports = reportsByStage[stage]
                            .filter((r) =>
                                activeReviewStageTab === 'ALL'
                                    ? r
                                    : allReportReviews[r.reviewId]?.stage ===
                                      activeReviewStageTab,
                            )
                            .filter((r) =>
                                            ...
                                            {reports.length === 0 ? (
                                                ...
                                                reports.map((report) => {
                                                    ...
                                                    const rateeFullName =
                                                        ...
                                                        `${allReviewRateeUsers[rateeId]?.lastName} ${allReviewRateeUsers[rateeId]?.firstName}`;
                                                    const teamTitle =
                                                        allReviewRateeUsers[
                                                            rateeId
                                                        ]?.teamTitle ?? null;
