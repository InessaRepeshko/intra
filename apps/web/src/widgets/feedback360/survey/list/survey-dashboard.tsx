...
import { ResponseStatusBadge, responseStatusConfig } from '@/entities/feedback360/respondent/ui/response-status-badge';
...
export function SurveyDashboard({
    ...
    const [activeCycleTab, setActiveCycleTab] = useState<CycleTabType>(cycleTabOptions[0]);
    ...
    const displayedSurveysData = useMemo(() => {
        return reviewRespondentsData.flatMap((r) => {
            const review = allReviewsData.find(
                (rev) => rev.id === r.reviewId,
            );
            if (!review) return [];

            const matchingRespondents =
                isMyTeamSurveys
            ...
            const cycle = allCyclesData?.find(
    ...
    const surveysByStatus = useMemo(() => {
        const surveys = displayedSurveysData.sort((a, b) => (b.review?.id ?? 0) - (a.review?.id ?? 0));
        ...
        surveys.forEach((r) => {
            if (r.respondent?.responseStatus && buckets[r.respondent?.responseStatus]) {
    ...
    return (
        <Card className="mx-auto gap-6 sm:gap-8 flex flex-col w-full h-full border-border p-4 sm:p-6 md:p-8 overflow-hidden">
                ...
                <div>
                    ...
                    <p className="mt-1 text-muted-foreground">
                        ...
                        <span className="font-medium text-foreground">
                            {surveysByStatus[ResponseStatus.PENDING]?.length.toString() ?? '0'}
                        </span>{' '}
                        {isMyTeamSurveys
            ...
            <div className="flex flex-row flex-wrap w-full gap-3 sm:gap-4 md:gap-6 justify-around">
                <StatisticsCard
                    title={`Pending`}
                    value={surveysByStatus[ResponseStatus.PENDING]?.length.toString() ?? '0'}
                    icon={Hourglass}
                ...
                <StatisticsCard
                    title={`In Progress`}
                    value={surveysByStatus[ResponseStatus.IN_PROGRESS]?.length.toString() ?? '0'}
                    icon={PencilIcon}
                ...
                <StatisticsCard
                    title={`Completed`}
                    value={surveysByStatus[ResponseStatus.COMPLETED]?.length.toString() ?? '0'}
                    icon={SquareCheck}
            ...
            <Tabs
                ...
                onValueChange={(v) => setActiveResponseStatusTab(v as ResponseStatus | 'ALL')}
            ...
            >
                ...
                >
                    {responseStatusTabOptions.map((status) => (
                        <TabsTrigger
                            ...
                            className="rounded-xl text-sm whitespace-nowrap text-center"
                        >
                            {status === 'ALL' ? 'All statuses' : responseStatusConfig[status].label}
                        </TabsTrigger>
                    ))}
                </TabsList>
                ...
                {responseStatusTabOptions.map((status) => {
                    ...
                    const surveys = surveysByStatus[status]
                        .filter((r) =>
                            activeResponseStatusTab === 'ALL' ? r : r.respondent?.responseStatus === activeResponseStatusTab
                        )
                        .filter((r) =>
                            activeCycleTab.value === -1 ? r :
                                activeCycleTab.value === 0 ? (r.review?.cycleId === null || r.review?.cycleId === undefined) :
                                    r.review?.cycleId === activeCycleTab.value ? r : null
                        )
                        .filter((r): r is SurveyInfo => r !== null && r !== undefined);
                                    ...
                                    <div className="space-y-6">
                                        {surveys.length === 0 ? (
                                            <div className="py-12 text-center text-muted-foreground">
                                                No surveys in this status for the selected cycle
                                            </div>
                                        ) : (
                                            surveys.map((survey) => {
                                                            ...
                                                            <div className="space-y-1 flex-1 min-w-0 w-full">
                                                                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                                                                    <p className="font-medium text-lg text-foreground break-words">
                                                                        <span className="text-muted-foreground border border-border rounded-xl px-1 bg-neutral-100">#{survey.review?.id}-{survey.respondent?.id}</span>{" "}
                                                                        {
                                                                            survey
                                                                                .review
                                                                                ?.rateeFullName
                                                                    ...
                                                                    />
                                                                    <ResponseStatusBadge status={survey.respondent?.responseStatus} />
                                                        ...
                                                        <div className="flex flex-row items-center gap-x-8 gap-y-2 w-full flex-wrap justify-center sm:justify-end">
                                                            <div className="flex items-center gap-x-1 gap-y-0 text-base flex-wrap justify-center lg:justify-end">
                                                                ...
                                                                <span className="text-muted-foreground whitespace-nowrap">
                                                                    {survey.review.cycleId && survey.respondent.responseStatus === ResponseStatus.COMPLETED ? 'Completed' :
                                                                        !survey.review.cycleId ? 'Created' : 'Due'}
                                                                </span>
                                                                <span className="font-medium text-foreground whitespace-nowrap">
                                                                    {survey.review.cycleId ? (
                                                                        format(
                                                                            survey.cycle?.responseDeadline ||
                                                                            survey.cycle?.reviewDeadline ||
                                                                            survey.cycle?.endDate ||
                                                                            '',
                                                                            'MMM dd, yyyy',
                                                                        )
                                                                    ) : (format(
                                                                        survey.review.createdAt,
                                                                        'MMM dd, yyyy',
                                                                    )
                                                                    )}
                                                                </span>
                                                            ...
                                                            >
                                                                {isCurrentUserRespondent && survey.review.stage === ReviewStage.IN_PROGRESS
                                                                    && (survey.respondent.responseStatus === ResponseStatus.PENDING
                                                                        || survey.respondent.responseStatus === ResponseStatus.IN_PROGRESS) ? (
                                                                    <Link
