...
import { StageBadge, stageConfig } from '@entities/feedback360/review/ui/stage-badge';
...
export function ReviewsList() {
    ...
    const [activeTab, setActiveTab] = useState<ReviewStage>(ReviewStage.NEW);
    ...
    return (
        <main className="min-h-screen">
            <div className="mx-auto max-w-8xl gap-8 flex flex-col">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full grid-flow-col grid-rows-2 place-content-center">
                    <StatisticsCard
                        title={`New`}
                        value={
                            formatNumber(
                    ...
                    <StatisticsCard
                        ...
                        value={
                            formatNumber(
                                reviewsByStage[ReviewStage.WAITING_TO_START].length +
                    ...
                    <StatisticsCard
                        ...
                        value={
                            formatNumber(
                                ...
                                + reviewsByStage[ReviewStage.PREPARING_REPORT].length
                                + reviewsByStage[ReviewStage.PROCESSING_BY_HR].length,
                            ) ?? '-'
                        }
                        icon={ClipboardList}
                ...
                >
                    <TabsList className="flex flex-wrap h-auto justify-start gap-1 overflow-x-auto border rounded-xl">
                        {REVIEW_STAGE_ENUM_VALUES.map((stage) => (
                            <TabsTrigger key={stage} value={stage} className="border rounded-xl">
                            ...
                            <Card className="bg-card border-border">
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        ...
                                        {reviewsByStage[stage].length !== 1 ? "reviews are" : "review is"} {' '}
                                        currently at the{' '}
                                        ...
                                        {reviewsByStage[stage].length === 0 ? (
                                            ...
                                            reviewsByStage[stage].map(
                                                (review) => {
                                                    ...
                                                    const cycle = currentCycles.find((c) => c.id === review.cycleId);
                                                    return (
                                                        <div
                                                            key={review.id}
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-6 shrink-0 w-full md:w-auto">
                                                                <div className="flex items-center gap-x-1 text-base flex-wrap justify-center md:justify-end">
                                                                    <Hourglass className="shrink-0 h-4 w-4 text-muted-foreground" />
                                                                    <span className="font-medium text-foreground whitespace-nowrap">
                                                                        {Number(respondentCounts[review.id] ?? 0) - Number(answerCounts[review.id] ?? 0)}
                                                                    </span>
                                                                ...
                                                                <div className="flex items-center gap-x-2 text-base flex-wrap justify-center md:justify-end">
                                                                    ...
                                                                    <span className="font-medium text-foreground whitespace-nowrap">
                                                                        {format(cycle?.responseDeadline || cycle?.reviewDeadline || cycle?.endDate || '', 'MMM dd, yyyy')}
                                                                    </span>
                                                                </div>
                                                                <Button
                                                                    asChild
