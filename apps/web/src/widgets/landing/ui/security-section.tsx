import { Eye, Key, Lock, ShieldCheck } from 'lucide-react';
...
const securityItems = [
    {
        ...
        description:
            'HTTP-only cookies with strict session management protect every interaction.',
    },
    {
        icon: Eye,
        title: 'Anonymised Feedback',
        description:
            'Text responses are aggregated and anonymised to protect respondent identity.',
    },
    {
        icon: ShieldCheck,
        title: 'Guard-Protected API',
        description:
            'AuthSession and Roles guards enforce authentication and authorization on every endpoint.',
    },
    {
        icon: Key,
        title: 'Google OAuth2',
        description:
            'Enterprise-grade authentication via Google workspace. No passwords to manage.',
    },
];
                ...
                <div className="grid gap-16 lg:grid-cols-2 lg:items-center">
                    ...
                    <div>
                        ...
                        <p className="mt-4 max-w-lg text-lg leading-relaxed text-muted-foreground text-pretty">
                            Your people data is sensitive. Intra is built with
                            security at every layer - from authentication to
                            data anonymisation.
                        </p>
                    </div>
                    ...
                    <div className="grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 bg-gradient-to-br from-white/60 to-white/30 transition-all overflow-hidden">
                        {securityItems.map((item, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon
                                        className={`h-5 w-5 ${isEven ? 'text-pink-600' : 'text-amber-600'}`}
                                    />
