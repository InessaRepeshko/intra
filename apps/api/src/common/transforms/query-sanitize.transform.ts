...
export function ToOptionalTrimmedString(options?: {
    min?: number;
    max?: number;
}): PropertyDecorator {
 ...
 */
export function ToOptionalInt(options?: {
    min?: number;
    max?: number;
}): PropertyDecorator {
    return Transform(({ value }) => {
...
export function ToOptionalEnum<TEnum extends object>(
    _enum: TEnum,
): PropertyDecorator {
    return Transform(({ value }) => {
        if (isEmpty(value)) return undefined;
