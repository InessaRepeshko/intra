...
export class PositionHierarchyService {
    constructor(
        @Inject(ORGANISATION_POSITION_HIERARCHY_REPOSITORY)
        private readonly hierarchy: PositionHierarchyRepositoryPort,
    ...
    async link(
        ...
        if (superiorPositionId === subordinatePositionId) {
            throw new BadRequestException(
                'Position cannot be a subordinate of itself',
            );
        }
    ...
    async listSubordinates(
        ...
        await this.positions.getById(superiorPositionId);
        const relations =
            await this.hierarchy.listSubordinates(superiorPositionId);
        return Promise.all(
            relations.map((rel) =>
                this.positions.getById(rel.subordinatePositionId),
            ),
        );
    }
    ...
    async listSuperiors(
        ...
        await this.positions.getById(subordinatePositionId);
        const relations = await this.hierarchy.listSuperiors(
            subordinatePositionId,
        );
        return Promise.all(
            relations.map((rel) =>
                this.positions.getById(rel.superiorPositionId),
            ),
        );
    }
}
