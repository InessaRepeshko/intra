...
export class ReviewQuestionRelationQueryDto {
    ...
    @ApiPropertyOptional({ example: 1, description: 'Question id', type: 'number' })
    @ToOptionalInt({ min: 1 })
    @IsOptional()
    @IsInt()
    questionId?: number;
    ...
    @ApiPropertyOptional({ example: AnswerType.NUMERICAL_SCALE, description: 'Answer type', type: 'string' })
    @ToOptionalEnum(AnswerType)
    @IsOptional()
    @IsEnum(AnswerType)
    answerType?: AnswerType;

    @ApiPropertyOptional({ example: 1, description: 'Competence id', type: 'number' })
    @ToOptionalInt({ min: 1 })
    @IsOptional()
    @IsInt()
    competenceId?: number;
    ...
    @ApiPropertyOptional({ example: true, description: 'Is for selfassessment', type: 'boolean' })
    @ToOptionalBool()
    @IsOptional()
    @IsBoolean()
    isForSelfassessment?: boolean;

    @ApiPropertyOptional({ example: ReviewQuestionRelationSortField.QUESTION_TITLE, enum: ReviewQuestionRelationSortField, description: 'Sort field by ', type: 'string', default: ReviewQuestionRelationSortField.ID })
