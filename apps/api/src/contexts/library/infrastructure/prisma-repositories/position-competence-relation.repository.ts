...
export class PositionCompetenceRelationRepository implements PositionCompetenceRelationRepositoryPort {
  ...
  async link(competenceId: number, positionId: number): Promise<PositionCompetenceRelationDomain> {
    try {
      const relation = await this.prisma.positionCompetenceRelation.create({
        data: { competenceId, positionId },
      });
      return LibraryMapper.toPositionCompetenceRelationDomain(relation);
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        const existing = await this.prisma.positionCompetenceRelation.findUnique({
          where: { positionId_competenceId: { positionId, competenceId } },
        });
        if (existing) return LibraryMapper.toPositionCompetenceRelationDomain(existing);
      }
      throw error;
    }
  }
  ...
  async listByCompetence(competenceId: number): Promise<PositionCompetenceRelationDomain[]> {
  ...
  async listByPosition(positionId: number): Promise<PositionCompetenceRelationDomain[]> {
    const relations = await this.prisma.positionCompetenceRelation.findMany({
      where: { positionId },
      orderBy: { createdAt: 'desc' },
    });
    ...
    await this.prisma.$transaction(async (tx) => {
      const deleteFilter =
        unique.length > 0
          ...
          : { competenceId };
      await tx.positionCompetenceRelation.deleteMany({ where: deleteFilter });

      if (unique.length) {
        await tx.positionCompetenceRelation.createMany({
          data: unique.map((positionId) => ({ competenceId, positionId })),
          skipDuplicates: true,
        });
      }
    });
