...
export class PositionQuestionTemplateRelationRepository implements PositionQuestionTemplateRelationRepositoryPort {
  ...
  async link(questionId: number, positionId: number): Promise<PositionQuestionTemplateRelationDomain> {
    try {
      const relation = await this.prisma.positionQuestionTemplateRelation.create({
        data: { questionTemplateId: questionId, positionId },
      });
      return LibraryMapper.toPositionQuestionTemplateRelationDomain(relation);
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        const existing = await this.prisma.positionQuestionTemplateRelation.findUnique({
          where: { questionTemplateId_positionId: { questionTemplateId: questionId, positionId } },
        });
        if (existing) return LibraryMapper.toPositionQuestionTemplateRelationDomain(existing);
      }
      throw error;
    }
  }
  ...
  async listByQuestion(questionId: number): Promise<PositionQuestionTemplateRelationDomain[]> {
    const relations = await this.prisma.positionQuestionTemplateRelation.findMany({
      where: { questionTemplateId: questionId },
      orderBy: { createdAt: 'desc' },
    });

    return relations.map(LibraryMapper.toPositionQuestionTemplateRelationDomain);
  }

  async replace(questionId: number, positionIds: number[]): Promise<PositionQuestionTemplateRelationDomain[]> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.positionQuestionTemplateRelation.deleteMany({ where: deleteFilter });
