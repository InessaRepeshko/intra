...
function buildReview(overrides: Partial<Parameters<typeof ReviewDomain.create>[0]> = {}): ReviewDomain {
...
function buildRespondent(overrides: Partial<Parameters<typeof RespondentDomain.create>[0]> = {}): RespondentDomain {
...
function buildUser(overrides: Partial<Parameters<typeof UserDomain.create>[0]> = {}): UserDomain {
...
describe('ReviewService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await Test.createTestingModule({
            providers: [
                ...
                { provide: QuestionTemplateService, useValue: questionTemplates },
    ...
    describe('getById', () => {
        ...
        it('returns the review when actor is the ratee', async () => {
            ...
            reviews.findById.mockResolvedValue(review);
            const actor = buildUser({ id: 200, roles: [IdentityRole.EMPLOYEE] });
            await expect(service.getById(1, actor)).resolves.toBe(review);
        });
        ...
        it('returns the review when actor is a respondent', async () => {
            ...
            const actor = buildUser({ id: 999, roles: [IdentityRole.EMPLOYEE] });
            await expect(service.getById(1, actor)).resolves.toBe(review);
        });
    ...
    describe('attachQuestion', () => {
        ...
        it('creates a new question snapshot when none exists', async () => {
            ...
            questions.create.mockImplementation(
                async (q: QuestionDomain) =>
    ...
    describe('addAnswer', () => {
        ...
        it('persists the answer when the review is IN_PROGRESS', async () => {
            ...
            answers.create.mockImplementation(async (a: any) => ({ ...a, id: 99 }));
    ...
    describe('addReviewer', () => {
        it('verifies the review exists before persisting', async () => {
            reviews.findById.mockResolvedValue(buildReview());
            reviewers.create.mockImplementation(
                async (r: ReviewerDomain) => r,
            );
    ...
    describe('listReviewers', () => {
        ...
        it('throws ForbiddenException when the actor has no relation to the review', async () => {
            ...
            respondents.listByReview.mockResolvedValue([]);
            reviewers.listByReview.mockResolvedValue([]);
            const actor = buildUser({ id: 9999, roles: [IdentityRole.EMPLOYEE] });
