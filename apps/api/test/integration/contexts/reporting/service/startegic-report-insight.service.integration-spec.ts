...
import { IdentityRole, InsightType, ReviewStage } from '@intra/shared-kernel';
...
describe('StartegicReportInsightService (integration)', () => {
    let module: TestingModule;
    let service: StartegicReportInsightService;
    let insightsRepo: StrategicReportInsightRepository;
    ...
    beforeAll(async () => {
        ...
        insightsRepo = module.get(StrategicReportInsightRepository);
    ...
    async function seedInsights() {
        await insightsRepo.createMany(strategicReportId, [
    ...
    describe('getByStrategicReportId', () => {
        it('returns insights tied to the strategic report for an HR actor', async () => {
            await seedInsights();
            ...
            expect(result[0].insightType).toBe(InsightType.HIGHEST_RATING);
        });

        it('returns an empty array when no insights exist (access still checked)', async () => {
            await expect(
    ...
    describe('getById', () => {
        it('returns the insight when found and the actor has access', async () => {
            await seedInsights();
        ...
        it('throws NotFoundException when insight id is missing', async () => {
            await expect(
