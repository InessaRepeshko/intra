...
describe('ReviewService', () => {
    ...
    describe('listQuestionRelations', () => {
        it('returns the question relations after the access check passes', async () => {
            const review = buildReview();
            reviews.findById.mockResolvedValue(review);
            const relations = [{ id: 1 }];
            questionRelations.listByReview.mockResolvedValue(relations);

            const actor = buildUser({ roles: [IdentityRole.HR] });
            const result = await service.listQuestionRelations(
                1,
                {} as any,
                actor,
            );

            expect(questionRelations.listByReview).toHaveBeenCalledWith(1, {});
            expect(result).toBe(relations);
        });
    });

    describe('listQuestions', () => {
        it('delegates to the questions repository', async () => {
            questions.search.mockResolvedValue([{ id: 1 }]);
            const result = await service.listQuestions({} as any);
    ...
    describe('listAnswers', () => {
        ...
        it('allows the manager of the review to read answers', async () => {
            const review = buildReview({ managerId: 5 });
            reviews.findById.mockResolvedValue(review);
            answers.list.mockResolvedValue([]);
            reviewers.listByReview.mockResolvedValue([]);
            ...
            await expect(
                service.listAnswers(1, undefined, actor),
            ).resolves.toEqual([]);
        });

        it('allows a respondent who is also a reviewer of the review', async () => {
            const review = buildReview({ rateeId: 999, managerId: 999 });
            reviews.findById.mockResolvedValue(review);
            answers.list.mockResolvedValue([]);
            reviewers.listByReview.mockResolvedValue([{ id: 1 }]);
    ...
    describe('countReviewAnswers', () => {
        ...
        it('allows the ratee of the review to count answers', async () => {
            const review = buildReview({ rateeId: 5 });
            reviews.findById.mockResolvedValue(review);
            answers.getAnswersCountByRespondentCategories.mockResolvedValue([]);
            ...
            await expect(service.countReviewAnswers(1, actor)).resolves.toBe(0);
        });
    });
