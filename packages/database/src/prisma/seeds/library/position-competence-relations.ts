...
const POSITION_COMPETENCES_SEED_DATA: PositionCompetenceSeed[] = [
    {
        positionTitle: 'CEO',
        competenceCodes: ['LEAD', 'COMM', 'PMGT', 'DELIV', 'QUAL', 'PROD', 'CUST', 'COLLAB', 'STRAT'],
    },
    {
        positionTitle: 'CTO',
        competenceCodes: ['LEAD', 'COMM', 'TECH', 'PMGT', 'DELIV', 'QUAL', 'PROD', 'COLLAB', 'STRAT'],
    },
    {
        positionTitle: 'Team Lead',
        competenceCodes: ['LEAD', 'COMM', 'TECH', 'PMGT', 'DELIV', 'QUAL', 'COLLAB'],
    },
    {
        positionTitle: 'Tech Lead',
        competenceCodes: ['LEAD', 'COMM', 'TECH', 'PMGT', 'DELIV', 'QUAL', 'COLLAB', 'STRAT'],
    },
    {
        positionTitle: 'Senior Software Engineer',
        competenceCodes: ['COMM', 'TECH', 'DELIV', 'QUAL', 'PROD', 'CUST', 'COLLAB', 'STRAT', 'LEAD', 'PMGT'], // All 10
    },
    {
...
export default async function seedPositionCompetenceRelations(
    ...
    for (const item of POSITION_COMPETENCES_SEED_DATA) {
        ...
        if (!position) {
            console.warn(`⚠️ Position ${item.positionTitle} not found, skip competence links`);
            continue;
        }

        for (const code of item.competenceCodes) {
            const competence = await prisma.competence.findUnique({ where: { code } });
            if (!competence) {
                console.warn(`⚠️ Competence ${code} not found, skip link for ${item.positionTitle}`);
                continue;
            }
