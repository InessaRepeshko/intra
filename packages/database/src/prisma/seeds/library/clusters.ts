...
const CLUSTER_LEVELS = [
    { lowerBound: 0, upperBound: 1, title: 'Beginner', description: 'Initial level of competence development' },
    { lowerBound: 1, upperBound: 2, title: 'Novice', description: 'Basic understanding and application' },
    { lowerBound: 2, upperBound: 3, title: 'Intermediate', description: 'Confident application in standard situations' },
    { lowerBound: 3, upperBound: 4, title: 'Advanced', description: 'Proficient with ability to handle complex scenarios' },
    { lowerBound: 4, upperBound: 5, title: 'Expert', description: 'Mastery level with innovative approaches' },
];
...
export default async function seedClusters(prisma: PrismaClient) {
    for (const group of CLUSTERS_SEED_DATA) {
        ...
        if (!competence) {
            console.warn(`⚠️  Competence not found for clusters: ${group.competenceCode}`);
            continue;
        }
